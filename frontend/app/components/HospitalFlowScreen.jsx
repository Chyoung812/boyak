...
function HospitalFlowScreen({
  ...
  return (
    <section aria-labelledby="hospital-flow-title">
      ...
      {step === "select" && (
        <HospitalSelectPanel hospital={hospital} onStepChange={onStepChange} />
      )}
...
function SymptomSelectPanel({ selectedSymptom, onSelect, onSpeak }) {
  ...
  return (
    <div className="mx-auto max-w-[560px] rounded-[30px] border-2 border-boyak-line bg-white px-7 py-8 shadow-soft sm:px-9 sm:py-10 lg:max-w-[720px] lg:px-6 lg:py-5">
      ...
      {/* 음성 안내 문구 */}
      <p className="mb-3 text-center text-lg font-bold text-boyak-muted lg:mb-2 lg:text-base">
        아래 증상 외에 다른 증상을 말하셔도 돼요
      </p>
      ...
      >
        ...
        {liveText && (
          <span className="mt-1 text-xl font-bold text-boyak-green lg:text-base">&ldquo;{liveText}&rdquo;</span>
        )}
      </button>
...
// ─── 병원 선택 패널 ────────────────────────────────────────────────────────────
function HospitalSelectPanel({ hospital, onStepChange }) {
  return (
