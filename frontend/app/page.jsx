...
const STATIC_AUDIO = {
  ...
  "많이 물어보는 병원비를 미리 정리한 화면이에요. 초음파, 물리치료, 주사치료 같은 항목별 예상 비용과 병원에 물어볼 말을 확인하세요.": "/audio/cost_chat_friendly.wav",
...
export default function Home() {
  ...
  const getGlobalVoiceGuide = useCallback(() => {
    ...
    if (view === "cost") {
      ...
      if (costStep === "chat") {
        return isSimpleVoice
          ? "많이 물어보는 병원비를 확인하세요."
          : "많이 물어보는 병원비를 미리 정리한 화면이에요. 초음파, 물리치료, 주사치료 같은 항목별 예상 비용과 병원에 물어볼 말을 확인하세요.";
      }
  ...
  const handleCostAsk = useCallback(() => {
    speak("궁금한 병원비를 말씀해주세요. 예를 들어, 초음파 검사는 얼마인가요 라고 말할 수 있어요.");
  }, [speak]);
