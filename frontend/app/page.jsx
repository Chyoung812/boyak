...
const STATIC_AUDIO = {
  ...
  "급여기준으로 설명 가능한 기본 비용만 정리했어요. 기본 물리치료와 X-ray 비용, 병원에 물어볼 말을 확인하세요.": "/audio/cost_chat_friendly.wav",
...
export default function Home() {
  ...
  const getGlobalVoiceGuide = useCallback(() => {
    ...
    if (view === "cost") {
      ...
      if (costStep === "chat") {
        return isSimpleVoice
          ? "많이 물어보는 병원비를 확인하세요."
          : "급여기준으로 설명 가능한 기본 비용만 정리했어요. 기본 물리치료와 X-ray 비용, 병원에 물어볼 말을 확인하세요.";
      }
  ...
  const handleCostAsk = useCallback(() => {
    speak("궁금한 병원비를 말씀해주세요. 예를 들어, X-ray는 얼마인가요 라고 말할 수 있어요.");
  }, [speak]);
