...
_PUBLIC_DATA_NOTES = {
    "11990": "수가기준정보: 진찰료·X-ray·기본 물리치료 같은 급여 행위의 기준금액/행위코드 확인용",
    "13000": "진료행위정보: 어떤 행위가 실제로 많이 쓰이는지 보는 행위 통계/맥락용",
    "13200/13201": "진료비통계: 의료기관종별·진료과 전체 청구통계. 초진 진찰비로 쓰면 안 됨",
    "11997": "비급여 진료비 정보: 도수치료·체외충격파·일부 초음파/MRI 등 HIRA 공개 비급여 가격 확인용",
}
...
def answer_cost_question(question: str, body: str = "통증", treatment: str = "진찰 + X-ray + 처방전 받을 수 있음") -> Dict[str, Any]:
    ...
    if any(k in q for k in ["진료과", "무슨 과", "어느 과", "과목", "department"]):
        ...
        lines = [
            ...
            "HIRA 진료비통계의 진료과 예시는 " + ", ".join(DEPARTMENT_EXAMPLES) + " 등이에요.",
            "단, 이 통계는 진료과 전체 평균/청구통계라서 ‘초진 진찰비 얼마’로 쓰면 안 돼요.",
        ]
        if any(k in q for k in ["수가", "11990", "급여기준", "급여 기준", "행위", "코드"]):
            sources.append("11990")
            lines.append("급여기준/수가 질문은 11990으로 진찰료·X-ray·기본 물리치료 같은 행위 기준을 확인하는 방식으로 연결합니다.")
        followups = ["이 증상은 어느 과가 맞나요?", "진료과 통계는 초진비로 못 쓰나요?"]
    elif any(k in q for k in ["수가", "11990", "급여기준", "급여 기준", "행위", "코드"]):
        topic = "covered_fee_basis"
        sources = ["11990", "13000"]
        lines = [
            "11990 수가기준정보는 진찰료·X-ray·기본 물리치료 같은 ‘급여 행위’의 기준금액을 확인하는 근거예요.",
            "현재 화면의 기본 가이드는 진찰 → X-ray/기본검사 → 처방전 받을 수 있음 → 기본 물리치료 같은 첫 방문 흐름을 11990 행위 기준으로 설명해요.",
            "다만 개인 최종 병원비는 행위 조합, 급여 기준 충족, 병원 종별, 야간/공휴일, 추가 비급여 여부에 따라 달라져요.",
        ]
        followups = ["X-ray도 급여인가요?", "물리치료 기준은 뭐예요?", "비급여는 어디서 봐요?"]
        ...
        sources = ["11997", "11990"]
        lines = [
            "MRI는 무조건 비급여라고 단정하면 안 되고, 증상·진찰소견·기존 검사 결과에 따라 급여 가능성이 달라져요.",
            "첫 방문에서는 보통 진찰과 X-ray 같은 기본검사 후 필요성을 확인하는 흐름이 안전합니다.",
            "병원에는 ‘MRI가 급여 기준에 해당하나요? 비급여라면 1회 금액과 급여 대안이 있나요?’라고 물어보세요.",
        ]
        followups = ["MRI 급여 기준", "비급여 가격은 어디서 확인해요?", "실손 청구는 어떻게 물어봐요?"]
    elif any(k in q for k in ["실손", "보험", "환급", "청구", "공제"]):
        topic = "insurance"
        sources = ["11997"]
        lines = [
            "HIRA 공개자료에는 개인 실손보험 지급액·공제금액·거절 사유가 들어있지 않아요.",
        ...
        sources = ["11997", "11990"]
        lines = [
            "도수치료·체외충격파·일부 주사/초음파/MRI는 병원별 비급여 가격 차이가 클 수 있어요.",
            "HIRA 11997은 ‘HIRA 공개 비급여 가격’을 보는 자료이고, 최종 실손 환급액은 포함하지 않아요.",
        ...
        followups = ["약국비도 포함해요?", "처방전 받을 수 있음이 뭐예요?"]
        ...
        lines = [
            f"{body_label} 첫 방문은 보통 진찰 → 필요 시 X-ray/기본검사 → 처방전 받을 수 있음 → 기본 물리치료 여부 확인 순서로 생각하면 돼요.",
            f"선택한 항목: {treatment}. 화면 금액은 확정 병원비가 아니라 첫 방문 전 안심 가이드입니다.",
            "추가 검사·주사·도수치료 같은 항목을 권유받으면 급여/비급여, 1회 금액, 예상 횟수, 급여 대안을 물어보세요.",
        ]
        followups = ["MRI도 건강보험 돼요?", "진료과 통계는 어떻게 써요?", "비급여면 실손 되나요?"]
